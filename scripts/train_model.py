...
====================================
Train a model to predict the importance of DOM nodes and export it in ONNX format.
...
Labels:
  Importance score from 0.0 to 1.0 (continuous regression)
"""
...
# For real training, use crawled data from sites with correct aria-* attributes.
# ================================================================

def generate_synthetic_dataset(n_samples: int = 50000, seed: int = 42) -> tuple:
    """
Generate synthetic data using heuristic rules as teacher labels.
    For real training:
    1. Storybook site DOM -> components with correct aria-* attributes
    2. Crawled data from WCAG-compliant sites
    """
    ...
    for i in range(n_samples):
        ...
        # Tag category (mutually exclusive)
        tag_roll = rng.random()
        ...
        # ---- Compute ground truth score (using heuristic model as teacher) ----
        score = _heuristic_score(f)

        # Add small noise to improve generalization
...
def _heuristic_score(f: dict) -> float:
    """Heuristic score corresponding to FeatureExtractor."""
    score = 0.0
...
def train(n_samples: int = 50000):
    ...
    # ---- Model definition ----
    # GradientBoosting: better generalization than pure heuristics
    model = Pipeline([
    ...
    # Threshold accuracy (evaluated as classification task)
    threshold = 0.3
